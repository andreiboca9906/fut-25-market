...
class FutClient:
    ...
    async def login(
        ...
        two_factor_provider: Optional[TwoFactorCodeProvider] = None
    ) -> LoginResponse:
    ...
    async def search_players(self, params: PlayerSearchParameters) -> SearchResponse:
        ...
        try:
            response = await self.session.get(
                f"{self.base_url}{API_ENDPOINTS['transfermarket']}",
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                return BidResult(
                    success=False,
                    current_bid=0,
                    error_message="Auction expired or invalid"
                )
            elif response.status_code == 461:
                return BidResult(
                    success=False,
                    current_bid=0,
                    error_message="Bid too low"
                )
    ...
    async def buy_now(self, trade_id: int) -> BidResult:
        ...
        try:
            ...
            return BidResult(
                success=True,
                current_bid=data["auctionInfo"][0]["buyNowPrice"],
                auction_info=None
            )
    ...
    async def send_to_club(self, item_id: int) -> SendToClubResult:
        ...
        try:
            response = await self.session.put(
                f"{self.base_url}{API_ENDPOINTS['send_to_club']}",
                json={"itemData": [{"id": item_id, "pile": "club"}]}
            )
    ...
    async def relist_items(self) -> RelistResult:
        ...
        try:
            ...
            return RelistResult(
                success=True,
                relisted_count=data.get("count", 0),
                errors=[]
            )
    ...
    async def add_to_watchlist(self, trade_id: int) -> bool:
        ...
        try:
            response = await self.session.put(
                f"{self.base_url}{API_ENDPOINTS['add_to_watchlist']}",
                json={"auctionInfo": [{"id": trade_id}]}
            )
